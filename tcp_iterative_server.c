...
#define MAXLINE 512
#define CONN_LIMIT 3 // Number of clients connected at a time
...
// Wait for connections from client processes. Example of an iterative server.
for (connCount = 0; connCount < CONN_LIMIT; connCount++) 
{
    clilen = sizeof(cli_addr);
    socketArray[connCount] = accept(sockfd,(struct sockaddr *) &cli_addr, &clilen);
    if (socketArray[connCount] < 0)
...
for (connCount = 0; connCount < CONN_LIMIT; connCount++) 
{
    interact(socketArray[connCount]);
}

close(sockfd);
for (int connCount=0; connCount < CONN_LIMIT; connCount++) 
    close(socketArray[connCount]);
